...
ci_lm<-function(fit,term){
 ...
 c(estimate=unname(cc[1]),se=unname(cc[2]),lower=unname(ci[1]),upper=unname(ci[2]),p=unname(cc[4]),df=fit$df.residual)
}
eff<-list()
for(s in rownames(H)){
 dd<-as.data.frame(p);dd$delta<-H[s,];w<-t.test(dd$delta[dd$arm=="golimumab"],dd$delta[dd$arm=="Placebo"])
 for(a in levels(p$arm))eff[[length(eff)+1]]<-data.table(set=s,contrast=paste0(a,"_within_change"),as.data.table(as.list(ci_t(dd$delta[dd$arm==a]))),method="one-sample t",unit="mean log2 post-minus-baseline")
 ...
 for(a in levels(p$arm)){
  ...
  eff[[length(eff)+1]]<-data.table(set=s,contrast=paste0(a,"_response_difference"),as.data.table(as.list(ci_lm(fit,"responseYes"))),method="OLS t",unit="difference of mean log2 changes")
 }
 fit<-lm(delta~arm+response,data=dd)
 eff[[length(eff)+1]]<-data.table(set=s,contrast="pooled_response_arm_adjusted",as.data.table(as.list(ci_lm(fit,"responseYes"))),method="OLS t",unit="arm-adjusted response difference in log2 change")
 fit<-lm(delta~arm*response,data=dd)
 eff[[length(eff)+1]]<-data.table(set=s,contrast="arm_response_interaction",as.data.table(as.list(ci_lm(fit,"armgolimumab:responseYes"))),method="OLS t",unit="difference of response differences")
}
effects<-rbindlist(eff,fill=TRUE);stopifnot(all(is.finite(effects$estimate)),all(is.finite(effects$p)),all(is.finite(effects$lower)),all(is.finite(effects$upper)));effects[,FDR:=p.adjust(p,"BH"),by=contrast]

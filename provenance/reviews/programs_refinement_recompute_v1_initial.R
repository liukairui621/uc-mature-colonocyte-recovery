...
for(model in unique(ctab$model))for(fam in unique(ctab$family)){
 t<-ctab[ctab$model==model&ctab$family==fam];if(nrow(t)) {bhchecks[[paste('001C',model,fam)]]<-max(abs(manual_bh(t$p)-t$FDR));families[[paste('001C',model,fam)]]<-nrow(t)}
}
for(model in unique(rtab$model)){t<-rtab[rtab$model==model];bhchecks[[paste('001C2',model)]]<-max(abs(manual_bh(t$p)-t$FDR_across_declared_programs));families[[paste('001C2',model)]]<-nrow(t)}
for(model in unique(ltab$model)){t<-ltab[ltab$model==model];bhchecks[[paste('001C_logistic',model)]]<-max(abs(manual_bh(t$LRT_p)-t$FDR))}
...
for(model in unique(htab$model)){t<-htab[htab$model==model];bhchecks[[paste('HC3',model)]]<-max(abs(manual_bh(t$HC3_p)-t$HC3_FDR))}
...
cv<-cov[match(coverage$program,cov$program)];coverage_match<-all(coverage$source==cv$source_genes)&all(coverage$measured==cv$measured_genes)&max(abs(coverage$fraction-cv$coverage))<1e-12

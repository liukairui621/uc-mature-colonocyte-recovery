...
def power(effect,se,df):
 crit=t.ppf(.975,df);return float(nct.cdf(-crit,df,effect/se)+nct.sf(crit,df,effect/se))
...
crit=t.ppf(.975,V['df']);direction={'at_assumed_discovery_effect':{'effect':D['estimate'],'two_sided_rejection_power':power(D['estimate'],V['SE'],V['df']),'positive_support_probability':positive_support(D['estimate'],V['SE'],V['df']),'negative_rejection_probability':float(nct.cdf(-crit,V['df'],D['estimate']/V['SE']))},'MDE80_two_sided_rejection':V['MDE80_plugin'],'MDE80_positive_support':mde(V['SE'],V['df'],True),'at_zero_effect':{'two_sided_rejection':power(0,V['SE'],V['df']),'positive_support':positive_support(0,V['SE'],V['df'])}}
